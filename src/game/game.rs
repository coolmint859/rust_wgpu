...
use crate::graphics::{
    camera::{Camera, Camera2D}, init_state::StateInit, material::{Material, SamplerComponent, TextureComponent}, mesh::Mesh, presets::{RenderPipeline, Shape2D, TextureSampler}, renderer::{Entity, Renderer}, traits::AppState, transform::Transform
};
...
impl Game {
    pub fn new() -> Self {
        ...
        map.insert(TextureSampler::NearestClampEdge.as_key(), 1);
        ...
        match material.add_component(SamplerComponent::new(TextureSampler::NearestClampEdge).with_bind_slot(1)) {
            Err(msg) => panic!("{}", msg),
            _ => {}
        }
