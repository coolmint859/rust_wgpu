...
fn vs_main(model: VertexInput, instance: InstanceInput) -> VertexOutput {
    ...
    // out.tex_coords = instance.bounds.xy * vec2<f32> (0.5, 0.5);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let distance_sample = textureSample(font_atlas, u_sampler, in.tex_coords);
    
    let text_color = vec4<f32>(1.0, 1.0, 1.0, distance_sample.r);

    return text_color * in.tint;
}