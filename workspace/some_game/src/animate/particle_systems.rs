...
use graphics::{data_table::DataTable, instance::InstanceGroup, transform::Transform, vertex::attr};

use crate::{animate::{animation::FadeMode, particle_emitter::{ParticleBehavior, ParticleInit, ParticleLifeCycle}}};
...
impl ParticleLifeCycle for PointSpawner2D {
    fn spawn(&self, instances: &mut InstanceGroup, particles: &mut DataTable, range: Range<usize>) {
        let transforms_opt = instances.get_attribute_mut::<Transform>(attr::TRANSFORM);
...
impl ParticleBehavior for FadeBehavior {
    fn simulate(&self, instances: &mut InstanceGroup, particles: &mut DataTable, count: usize, _dt: f32) {
        ...
        let tints_opt = instances.get_attribute_mut::<Vec4>(attr::TINT_COLOR);
...
impl ParticleLifeCycle for PlaneSpawner {
    fn spawn(&self, instances: &mut InstanceGroup, _properties: &mut DataTable, range: Range<usize>) {
        if let Some(transforms) = instances.get_attribute_mut::<Transform>(attr::TRANSFORM) {
            let rng = &mut rand::thread_rng();
