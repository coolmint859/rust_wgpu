use std::{collections::HashMap, fs::File, io::Read, sync::{Arc, atomic::{AtomicBool, Ordering}}};
use rayon::prelude::*;
use glam::Vec4;

use crate::{
    geometry::Geometry, 
    handler::{BuilderType, ResourceBuilder}, 
    instance::InstanceGroup, 
    presets::{MaterialPreset, RenderPipeline, ShaderSpecPreset}, 
    primitive::{Primitive, RenderInfo}, 
    shape_factory::Shape2D, 
    texture::{TextureBuilder, TextureContext}, 
    transform::Transform, 
    vertex::*, 
    wpgu_context::*
};

/// Data struct containing the sdf settings for a font
#[derive(Clone, Debug)]
...
pub struct Font {
    ...
    id: ResourceID,
    /// the path to the source ttf file
    path: String, 
    /// the font settings used to create the associated font asset
    settings: FontSettings,
}

impl Font {
    pub fn new(path: &str, settings: FontSettings) -> Self {
        let id = ResourceID {
            key: path.to_string(),
        ...
        Self { settings, id, path: path.to_string() }
    }
    ...
    pub fn create_primitive(&self, instance_cap: usize) -> Primitive {
        ...
        let instances = InstanceGroup::new(0, instance_cap) // allow space for 1000 characters
            .with_label(&format!("font::{}", self.path))
        ...
        let label = format!("font::{}", self.path);
    ...
    pub fn get_builder(&self) -> FontBuilder {
        FontBuilder::new(self.path.clone(), self.settings.clone())
    }
    ...
    /// get a copy of the settings for this font
...
impl FontBuilder {
    pub fn new(path: String, settings: FontSettings) -> Self {
        Self { path, settings }
    }
}
...
impl ResourceBuilder for FontBuilder {
    ...
    fn hold_time(&self) -> Option<u64> { HoldPolicy::Transient.as_seconds() }
    fn builder_type(&self) -> BuilderType { BuilderType::Blocking }
    ...
    fn build(&self, context: Arc<Self::Context>, cancel_flag: Arc<AtomicBool>) -> Result<Self::Output, String> {
        ...
        let line_metrics = font.horizontal_line_metrics(self.settings.scale)
            .ok_or("Failed to read font line metrics.")?;
        let line_height = line_metrics.new_line_size / self.settings.scale;

        if cancel_flag.load(Ordering::Relaxed) {
            return Err(format!("[Font] Main thread cancelled execution for font with path '{}'", self.path));
        }
        ...
        let atlas = TextureBuilder::new()
            ...
            .with_data(self.settings.atlas_size, self.settings.atlas_size, atlas_bytes)
            .with_format(wgpu::TextureFormat::R8Unorm)
            .build(context, cancel_flag)?;

        let font_asset = FontAsset { atlas, glyphs, line_height };

        println!("[Font] Created new font atlas and metadata from path '{}'.", self.path);
